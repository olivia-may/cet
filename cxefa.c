...
void replace_word(char * input_word, char * output_word)
{
	...
	if (is_matching_word_found == true)
	{
		for (int j = 0; j < output_word_len; j++)
		{
			if (i - j == 0) { break; }
			//printf("%d\n", j);
			output_stream[i - j] = output_word[output_word_len - 1  - j];
		}
	}
...
int main(int argc, char ** argv)
{
	...
	while (input_stream[i] != '\0')
	{
		...
		replace_word("tiu", "ths");
		replace_word("estas", "is a ");
		replace_word("testo", "test ");

		i++;
		output_stream = (char *)realloc(output_stream, (i + 2) * sizeof(output_stream));
	}
	output_stream[i] = '\0';
